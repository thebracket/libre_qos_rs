use crate::topology::{LqSite, LqAccessPoint};
use anyhow::Result;
...
impl NetworkNode {
    ...
    let mut js = String::new();
        ...
        js += &pad_line_add_eol(base+1, &format!("\"downloadBandwidthMbps\":{},", self.download_bandwidth_mbps));
        if self.children.is_empty() {
            js += &pad_line_add_eol(base+1, &format!("\"uploadBandwidthMbps\":{}", self.upload_bandwidth_mbps));
        } else {
            js += &pad_line_add_eol(base+1, &format!("\"uploadBandwidthMbps\":{},", self.upload_bandwidth_mbps));
        }
        if !self.children.is_empty() {
